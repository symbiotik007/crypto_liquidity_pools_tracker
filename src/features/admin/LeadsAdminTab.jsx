...
const STATUS_COLORS = {
  nuevo:      { bg: 'rgba(0,229,255,0.1)',   border: 'rgba(0,229,255,0.3)',   text: '#00e5ff' },
  contactado: { bg: 'rgba(255,179,71,0.1)',  border: 'rgba(255,179,71,0.3)',  text: '#ffb347' },
  convertido: { bg: 'rgba(0,255,136,0.1)',   border: 'rgba(0,255,136,0.3)',   text: '#00ff88' },
...
function LeadRow({ lead, onStatusChange }) {
  ...
  return (
    <>
      ...
      >
        <td style={{ padding: '12px 16px', fontSize: 13, color: '#c8e6f0', fontWeight: 600 }}>{lead.name}</td>
        <td style={{ padding: '12px 16px', fontSize: 12, color: '#4a7a96' }}>{lead.email || '—'}</td>
        <td style={{ padding: '12px 16px', fontSize: 11, color: '#2a5a72' }}>{date}</td>
      ...
      {open && (
        <tr style={{ background: '#070d14', borderBottom: '1px solid #0e2030' }}>
          <td colSpan={5} style={{ padding: '16px 20px' }}>
            <div style={{ fontSize: 12, color: '#4a7a96', marginBottom: 8, letterSpacing: 1, textTransform: 'uppercase' }}>Mensaje</div>
            <div style={{ fontSize: 14, color: '#c8e6f0', lineHeight: 1.65, whiteSpace: 'pre-wrap', marginBottom: 16 }}>
              {lead.message}
            </div>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <span style={{ fontSize: 11, color: '#2a5a72', letterSpacing: 1, textTransform: 'uppercase' }}>Cambiar estado:</span>
              ...
              {lead.email && (
                <a
                  ...
                  style={{ marginLeft: 'auto', fontSize: 12, color: '#00e5ff', textDecoration: 'none', fontWeight: 600 }}
                >
                  ✉ Responder por email →
                </a>
              )}
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <div style={{ fontSize: 16, fontWeight: 700, color: '#c8e6f0', marginBottom: 2 }}>Leads del formulario</div>
          <div style={{ fontSize: 12, color: '#2a5a72' }}>{leads.length} contacto{leads.length !== 1 ? 's' : ''} en total</div>
        </div>
      ...
      {/* Table */}
      {loading ? (
        <div style={{ textAlign: 'center', padding: 48, color: '#2a5a72', fontSize: 13 }}>Cargando leads...</div>
      ) : filtered.length === 0 ? (
        <div style={{ textAlign: 'center', padding: 48 }}>
          <div style={{ fontSize: 32, marginBottom: 12 }}>📭</div>
          <div style={{ fontSize: 14, color: '#2a5a72' }}>{filter === 'todos' ? 'No hay leads aún' : `No hay leads con estado "${filter}"`}</div>
        </div>
      ) : (

          ...
          {suggestions.map((s, i) => {
            ...
            return (
              <div key={i} className="email-sug-item" onMouseDown={() => select(s)}>
                <span style={{color:'var(--text-dim)'}}>{s.slice(0, atIdx + 1)}</span>
                <span style={{color:'var(--color-accent)', fontWeight:700}}>{s.slice(atIdx + 1)}</span>
              </div>
            )
          })}
        </div>
        ...
        <div className="m-body">
          ...
          <div className="m-tabs">
            {[['password','Contraseña'],['magic','Magic Link']].map(([k,l])=>(
              <button key={k} className="m-tab"
                ...
                style={{borderBottom:`2px solid ${tab===k?'var(--color-accent)':'transparent'}`,color:tab===k?'var(--color-accent)':'var(--text-dim)'}}>
                {l}
              </button>
            ))}
          </div>
          ...
          {tab==='password' && resetMode && !resetOk && (
            <form onSubmit={withReset} style={{display:'flex',flexDirection:'column',gap:14}}>
              <div style={{padding:'12px',background:'var(--bg-elevated)',border:'1px solid var(--border-blue)',fontSize:13,color:'var(--text-dim)',lineHeight:1.6}}>
          ...
          {resetOk&&<div className="m-ok">✉ Enlace enviado a<br/><strong style={{color:'var(--color-success)'}}>{email}</strong></div>}
          ...
          {magicOk&&<div className="m-ok">✉ Link enviado a<br/><strong style={{color:'var(--color-success)'}}>{email}</strong></div>}
      ...
      <div className="m-box">
        ...
        <div className="m-body">
          <div className="m-ok" style={{padding:24}}>
            ...
            <div style={{fontSize:13,color:'var(--color-success)',lineHeight:1.7}}>
              Enviamos un enlace de confirmación a<br/>
              <strong style={{color:'var(--color-success)'}}>{email}</strong><br/>
              <span style={{color:'var(--text-dim)'}}>Revisa tu bandeja y haz click en el enlace.</span>
            </div>
          </div>
        ...
        <div className="m-body">
          ...
          <form onSubmit={handleSignup} style={{display:'flex',flexDirection:'column',gap:14}}>
            ...
            <div style={{fontSize:11,color:'var(--text-dim)',textAlign:'center',lineHeight:1.6}}>
              Al registrarte aceptas nuestros <a href="#" style={{color:'var(--text-dim)'}}>Términos</a> y <a href="#" style={{color:'var(--text-dim)'}}>Privacidad</a>
            </div>
          </form>
